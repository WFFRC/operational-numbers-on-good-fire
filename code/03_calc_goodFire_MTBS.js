// Earth Engine Code
// code link: https://code.earthengine.google.com/219f9f57df135f2fbfd69e2506ea5f9c
// This script inputs fire perimeters and calculates low/mod vs. high severity fire for forested pixels. Severity (CBI) is derived from Parks et al.2019 method.

//----------- Settings for the analysis ------------------//
// var studyArea = states.filter(ee.Filter.inList('NAME',['Washington','Oregon','California','Idaho','Montana','Nevada','Utah','New Mexico','Colorado','Wyoming', 'Arizona']))
// var fires = severity.filter(ee.Filter.bounds(studyArea))
// var chunk = chunks.filter(ee.Filter.eq('chunk',1)).aggregate_array('Event_ID')
// var fires = severity.filter(ee.Filter.inList('Event_ID',chunk))//.filter(ee.Filter.gte('year','1995'))

var fires = MTBS.filter(ee.Filter.inList('Event_ID',['OR4499312125220140713'])) // Select study area
...
var indices = ee.ImageCollection(fires.map(function(ft){
 ...
 // Classify the image with the same bands used to train the Random Forest classifier.
  ...
  var CBIlowmod =  CBIbc.lt(2.25).and(CBIbc.gte(0.1)).updateMask(lcmsFor).rename('CBIlowmod'); // define refugia as bias corrected CBI less than this threshold (.1, .68, 1.25)
  var CBIunburned = CBIbc.lt(0.1).rename('CBIunburned').updateMask(lcmsFor); // define refugia as bias corrected CBI less than this threshold (.1, .68, 1.25)
  var CBIhighsev= CBIbc.gte(2.25).rename('CBIhighsev').updateMask(lcmsFor); // define refugia as bias corrected CBI less than this threshold (.1, .68, 1.25)
...
Map.addLayer(indices.select('frcLowCbiLow').first().gt(0).selfMask(),{min:0, max:1, palette:['yellow','orange']}, 'Good Fire, FRC low/mixed',false);
Map.addLayer(indices.select('CBIlowmod').first().gt(0).selfMask(),{min:0, max:1, palette:['yellow','orange']}, 'Good Fire, all FRC');
Map.addLayer(indices.select('CBIhighsev').first().gt(0).selfMask(),{min:0, max:1, palette:['yellow','red']}, 'High Severity Fire, all FRC');
