...
stateYearCompareLong <- stateYearCompare |>
    tidyr::pivot_longer(cols = c(CBIlowmodsev,
                               CBIhighsev,
                               rxBurnHa),
                                                    names_to = "datNm",
                                                    values_to = "hectares")
...
yearPerc <- stateYearCompare |>
  ...
  dplyr::group_by(year) |>
  dplyr::summarise(across(c(CBIlowmodsev, rxBurnHa), sum, na.rm=TRUE)) |>
  dplyr::mutate(RxPercOfLowMod = (rxBurnHa/CBIlowmodsev) * 100) |>
  dplyr::select(year, RxPercOfLowMod)
...
statePerc <- stateYearCompare |>
  ...
  dplyr::summarise(across(c(CBIlowmodsev, rxBurnHa), sum, na.rm=TRUE)) |>
  dplyr::mutate(RxPercOfLowMod = (rxBurnHa/CBIlowmodsev) * 100) |>
  dplyr::select(State, RxPercOfLowMod)
