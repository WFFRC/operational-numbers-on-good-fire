...
m <- leaflet::leaflet() %>%
  ...
  leaflet::addPolygons(data = gfSpatialForestOnly, fillColor = goodFCol, fillOpacity = 0.8, color = NA) %>%
...
#Get hex ids for future use
tlmr::col2hex("goldenrod2")
tlmr::col2hex("darkblue")
tlmr::col2hex("gray95")
tlmr::col2hex("gray50")
...
caseStudyEventFig <- tmap::tm_shape(caseStudyR, raster.downsample = FALSE) +
  tm_raster(palette = c("gray95", "#dc0ab4ff", "#FA6EE2", "goldenrod2")) +
  tmap::tm_shape(caseStudyBound) +
  tmap::tm_borders(col = 'gray50', lwd = 1) +
