...
_Code associated with this workflow is available in the GitHub repo [TylerLMcIntosh/a-number-on-good-fire](https://github.com/TylerLMcIntosh/a-number-on-good-fire). The code leverages functions from a personal function utilities package, "tlmr", available on GitHub at [TylerLMcIntosh/tlm-r-utility](https://github.com/TylerLMcIntosh/tlm-r-utility)._
...
library(tlmr)
tlmr::install_and_load_packages(c("dplyr",
                                  "rmarkdown",
                                  "purrr",
                                  "here",
                                  "quarto",
...
p1 <- tlmr::mblm_fit_estimator_and_visualize(dats = annual, x = year, y = cbiAnyBurned, repeated = FALSE)

p2 <- tlmr::mblm_fit_estimator_and_visualize(dats = annual, x = year, y = all_good_fire, repeated = FALSE)

# Perc siegel repeated medians estimators
p3 <- tlmr::mblm_fit_estimator_and_visualize(dats = annual, x = year, y = lowgf_perc_all_burned, repeated = FALSE)
p4 <- tlmr::mblm_fit_estimator_and_visualize(dats = annual, x = year, y = highgf_perc_all_burned, repeated = FALSE)
p5 <- tlmr::mblm_fit_estimator_and_visualize(dats = annual, x = year, y = allgf_perc_all_burned, repeated = FALSE)
...
```{r by-state}
tlmr::mblm_get_coefficients_by_group(dats = state_annual, x = year, y = allgf_perc_all_burned, group = NAME, repeated = FALSE) |>
...
west_allgf_perc_all_burned <- west |>
  dplyr::left_join(tlmr::mblm_get_coefficients_by_group(dats = state_annual, x = year, y = allgf_perc_all_burned, group = NAME, repeated = FALSE),
                   by = dplyr::join_by(NAME == group))
west_highgf_perc_all_burned <- west |>
  dplyr::left_join(tlmr::mblm_get_coefficients_by_group(dats = state_annual, x = year, y = highgf_perc_all_burned, group = NAME, repeated = FALSE),
                   by = dplyr::join_by(NAME == group))
west_lowgf_perc_all_burned <- west |>
  dplyr::left_join(tlmr::mblm_get_coefficients_by_group(dats = state_annual, x = year, y = lowgf_perc_all_burned, group = NAME, repeated = FALSE),
                   by = dplyr::join_by(NAME == group))
